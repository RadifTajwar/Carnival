...
export const getProducts = (keyword = '', currentPage = 1, price = [0, 25000], category, rating = 0) => async (dispatch) => {
    try {
        ...
        let link = `/api/v1/products?keyword=${keyword}&page=${currentPage}&product_price[gte]=${price[0]}&product_price[lte]=${price[1]}&product_rating[gte]=${rating}`; // sometimes a single '/' can cost a lot of time
        if (category) {
            link = `/api/v1/products?keyword=${keyword}&page=${currentPage}&product_price[gte]=${price[0]}&product_price[lte]=${price[1]}&product_category=${category}&product_rating[gte]=${rating}`
        }
